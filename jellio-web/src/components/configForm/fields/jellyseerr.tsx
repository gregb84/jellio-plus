...
export const JellyseerrFieldset: FC<Props> = ({ form }) => {
  ...
  return (
    <>
      {/* NEW SECTION: Stream Options */}
      <fieldset className="rounded-lg border p-2">
        <legend className="px-1 text-base font-medium">Stream Options</legend>
        <FormField
          control={form.control}
          name="directDownloadOnly"
          render={({ field }) => (
            <FormItem className="flex items-center justify-between py-2">
              <div>
                <FormLabel>Direct Download Only</FormLabel>
                <FormDescription>
                  Only provide direct download streams (no transcoded streaming option).
                </FormDescription>
              </div>
              <FormControl>
      ...
      {/* EXISTING: Jellyseerr integration */}
      <fieldset className="rounded-lg border p-2">
